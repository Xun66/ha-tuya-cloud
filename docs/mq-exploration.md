...
Use `tools/tuya_sharing_mq_probe.py` to test the official Home Assistant/Tuya
sharing authentication route. This is the same QR-login family used by Home
Assistant's built-in Tuya integration.
...
```bash
python3 -m pip install tuya-device-sharing-sdk qrcode[pil]
```
...
TUYA_DEVICE_ID=... \
TUYA_DURATION=300 \
python3 tools/tuya_sharing_mq_probe.py
```

The script prints the `tuyaSmart--qrLogin?...` payload, writes a QR image to
`/tmp/tuya-sharing-mq-login.png` when `qrcode` is available, starts the sharing
MQTT subscription after the app scan is accepted, and prints raw MQTT messages.
Use this route to verify whether the two-circuit meter publishes frequent
`protocol=4` reports through sharing MQTT.
...
- Home Assistant Tuya QR login flow: https://www.home-assistant.io/integrations/tuya/
